...
char *print_currently_serving(Ta *ta_list) {
	char *to_return = malloc(OUT_BUF_SIZE * sizeof(char));
    if (ta_list == NULL) {
        snprintf(to_return + strlen(to_return), OUT_BUF_SIZE - strlen(to_return), "No TAs are currently working.\n");
        return to_return;
    }
    while (ta_list != NULL) {
       if (ta_list->current_student != NULL) {
           snprintf(to_return + strlen(to_return), OUT_BUF_SIZE - strlen(to_return), "TA: %s is serving %s.\n",
                ta_list->name,
                ta_list->current_student->name);
       } else {
           snprintf(to_return + strlen(to_return), OUT_BUF_SIZE - strlen(to_return), "TA: %s has no student\r\n", ta_list->name);
       }
...
char *print_full_queue(Student *stu_list) {
	char *to_return = malloc(OUT_BUF_SIZE * sizeof(char));
    snprintf(to_return + strlen(to_return), OUT_BUF_SIZE - strlen(to_return), "Full Queue\n");
    while (stu_list != NULL) {
        snprintf(to_return + strlen(to_return), OUT_BUF_SIZE - strlen(to_return), "Student %s:%s\n",stu_list->name, stu_list->course->code);
        stu_list = stu_list->next_overall;
    }
	return to_return;
}
